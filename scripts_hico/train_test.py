...
import random
from losses import BCELoss_balanced
...
with open(all_data_dir + 'hico_infos/mask.pickle', 'rb') as fp:
    mask_f = pickle.load(fp, encoding='latin1')
mask_t = mask_f[0]
count_t = mask_f[1]
sigmoid = nn.Sigmoid()
...
loss_com_balanced = BCELoss_balanced()
        ...
        # ###Starting the iterations##################
            ...
            # import pdb;pdb.set_trace()........
                ...
                # ### Calculating Loss############
                    ...
                    #     / N_b
                    #----------------Modified Loss---------------------------------
                    loss_bce_balanced = torch.sum(loss_com_balanced(sigmoid(outputs)
                            * sigmoid(outputs_combine)
                            * sigmoid(outputs_single) * hum_obj_mask
                            * sigmoid(outputs_gem), labels.float(), ambiguity_score)) \
                        / N_b
                    
                    #--------------------------------------------------------------
                    # lossc = lossf.item()
                    lossc = loss_bce_balanced.item()
                
                    acc_epoch += lossc
                    iteration += 1
                    if phase == 'train' or phase == 'val':  # ### Flowing the loss backwards#########
                        # lossf.backward()
                        loss_bce_balanced.backward()
                        optimizer.step()

                # ##########################################################

                    # del lossf
                    del loss_bce_balanced
                    del model_out
                    del inputs
                    del outputs
                    del labels
