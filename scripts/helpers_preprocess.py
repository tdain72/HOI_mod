...
ANN_FILE_train = all_data_dir \
    + 'Annotations_vcoco/train_annotations_extra.json'
ANN_FILE_val = all_data_dir + 'Annotations_vcoco/val_annotations_extra.json'
ANN_FILE_test = all_data_dir + 'Annotations_vcoco/test_annotations_extra.json'
...
def get_compact_label(segment_key, flag):
    ...
    labels_np = np.zeros([no_person_dets, no_object_dets + 1,
                         NO_VERBS], np.int32)
    labels_object = np.zeros([no_person_dets, no_object_dets + 1], np.int32)
    ...
    if no_person_dets != 0 and len(a_p_boxes) != 0:
            ...
            if a_o_boxes and d_o_boxes:
                for do in range(len(d_o_boxes)):
                    ...
                    # enough iou
                        ...
                        labels_object[dd, do + 1] = OBJ2ID[current_hoi['obj_str']]
                        labels_np[dd, do + 1, verb_idx] = 1  # +1 because 0 is no object

        labels_object = labels_object.reshape(no_person_dets * (no_object_dets + 1))
        ...
        return {'labels_all': labels_np, 'labels_single': labels_single, 'labels_object': labels_object}
    else:
        labels_object = labels_object.reshape(no_person_dets * (no_object_dets + 1))
        comp_labels = labels_np.reshape(no_person_dets
                * (no_object_dets + 1), NO_VERBS)
...
def clean_up_annotation(annotation):
    ...
    for hoi in annotation:
        ...
        if len(objects['obj_bbx']) == 0:  # no obj case
            cur_oi = {'verb': hoi['Verbs'], 'obj_box': [], 'obj_str': []}
        else:

                        # 'obj_str': '',

            cur_oi = {'verb': hoi['Verbs'], 'obj_box': [int(coord)
                      for coord in hoi['object']['obj_bbx']], 'obj_str':  hoi['object']['obj_name']}
